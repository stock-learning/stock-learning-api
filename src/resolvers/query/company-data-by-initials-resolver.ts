...
import { CompanyDataDocument, ICompanyDataResource } from './../../documents/company-data-document';

class CompanyDataByInitialsResolver implements IResolver<IInitialsInputModel, ICompanyDataResource> {

    public readonly resolverName = 'companyDataByInitials';

    public async resolve(parent: any, args: IInitialsInputModel, context: GraphQLContext): Promise<ICompanyDataResource> {
        const doc = await CompanyDataDocument.findOne({ initials: args.initials });
        if (!doc) {
